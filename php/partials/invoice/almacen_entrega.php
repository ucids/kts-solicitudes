...
$query_gasto_mensual = "SELECT SUM(a.cantidad * a.precio) AS total_sum
    ...
    if ($status == 0){
        ...
        $status_value = 'Actualmente en Almacén';
	...
	if ($resultado_articulos->num_rows > 0) {
		...
		$query_solicitud = "UPDATE solicitudes SET 
            status = 7 , fecha_entrega = now()
            WHERE id = '$solicitud_id'";
		// $resultado_actualizar_solicitud = $conexion->query($query_solicitud);
		$fill = False;
	}
?>
										...
										<!--begin::Content-->
											<div class="flex-lg-row-fluid me-xl-18 mb-10 mb-xl-0">
												...
												<div class="mt-n1">
													...
													<!--end::Top-->
													<!--begin::Wrapper-->
													<form action="functions/invoice/almacen_entrega.php" method="POST">

													<div class="m-0">
														<!--begin::Label-->
														<div class="fw-bolder fs-3 text-gray-800 mb-8">Solicitud # <? echo $solicitud['id'] ?></div>
														<!--end::Label-->
														<!--begin::Row-->
														<div class="row g-5 mb-12">
															...
															<div class="col-sm-6">
																...
																<!--end::Label-->
																<!--end::Text-->
																<div class="fw-bolder fs-6 text-gray-800"><?php echo $solicitud['departamento']; ?></div>
																<!--end::Description-->
															</div>
															<!--end::Col-->
															<!--end::Col-->
															<div class="col-sm-6">
																...
																<!--end::Description-->
																<br>
																<div class="fw-bold fs-7 text-gray-600">Entrega de Mercancía
																<br /></div>
																<!--end::Description-->
															</div>
															<!--end::Col-->
																...
																<? if($fill){ ?>
																	<div class="alert alert-dismissible bg-light-success border border-success border-3 border-dashed d-flex flex-column flex-sm-row w-100 p-5 mb-10">
																		...
																		<div class="d-flex flex-column pe-0 pe-sm-10">
																			<h5 class="mb-1">Esta Orden Se ha Entregado con éxito</h5>
																...
																<table class="table">
																	<thead>
																		<tr class="border-bottom fs-6 fw-bolder text-muted text-uppercase">
																			...
																			<th class="min-w-70px pb-9 text-center">Cantidad Solicitada</th>
																			<th class="min-w-70px pb-9 text-center">Cantidad en Almacén</th>
																			<th class="min-w-70px pb-9 text-center">Cantidad en Entregada</th>
																		</tr>
																	</thead>
																	<tbody>
																		...
																		?>
																		<input type="hidden" name="id_articulo[<? echo $i; ?>]" value="<? echo $articulo['id'];?>">
																		<input type="hidden" name="cantidad_solicitada[<? echo $i; ?>]" value="<? echo $articulo['cantidad'];?>">
																		<input type="hidden" name="cantidad_entregada[<? echo $i; ?>]" value="<? echo $articulo['cantidad_entregada'];?>">
																		<input type="hidden" name="cantidad_recibida[<? echo $i; ?>]" value="<? echo $articulo['cantidad_recibida'];?>">

																		
																		<tr class="fw-bolder text-gray-700 fs-5 text-end">
																			<td class="d-flex align-items-center pt-11">
																			<i class="fa fa-genderless text-danger fs-1 me-2"></i><? echo $articulo['numero_parte']?></td>
																			<td class="pt-11"><? echo $articulo['descripcion']?></td>
																			<td class="pt-11"><? echo $articulo['unidad']?></td>
																			<td class="pt-11"><? echo $articulo['cantidad']?></td>
																			<td class="pt-11"><? echo $articulo['cantidad_recibida']?></td>
																			<?/*
																			<td class="pt-11">
																				<div class="form-check form-switch form-check-custom flex-center form-check-solid">
																					<input class="form-check-input" name="parcial[<?echo $i; ?>]" type="checkbox" value="1" 
																			...
																			<td class="pt-5	">
																				<input class="form-control form-control-solid" type="number" min="1" name="cantidad_entregada[<?echo $i ?>]" 
																				placeholder="<? echo $articulo['cantidad_entregada'] ?>" value="<? echo $articulo['cantidad_entregada']?>" 
											...
											<div class="m-0">
												...
												<div class="d-print-none border border-dashed border-gray-300 card-rounded h-lg-100 min-w-md-350px p-9 bg-lighten">
													...
													<div class="mb-6">
														...
														<div class="fw-bolder text-gray-800 fs-6"><? echo $solicitud['nombre'] . ' ' .$solicitud['apellidos']; ?></div>
													</div>
													...
													<div class="m-0">
														...
														<div class="mb-0">
															...
															<textarea name="notes" class="form-control opacity-100" rows="3" placeholder="Notas Importantes"></textarea>
														</div>
														<br>
															<input type="hidden" value="<? echo $user['id_user']?>" name = "usuario">
															<input type="hidden" value="<? echo $solicitud_id?>" name = "id_solicitud">
															<? if($fill){ ?>
																<a href="index.php" class="btn btn-primary"><i class="fas fa-envelope-open-text fs-4 me-2"></i> Inicio</a>
															<?}else{?>
																<button type="submit" class="btn btn-primary"><i class="fas fa-envelope-open-text fs-4 me-2"></i> Registrar</button>	
															<? }?>
															<div class="fw-bolder fs-6 text-gray-800 d-flex align-items-center">	
																...
																<? }else{ ?>
																	<? if ($rol == 4 && $status == 0){ ?>
																	<a href="functions/invoice/action.php?submit=aprove&id=
																		<?echo $solicitud_id?>&sup=<? echo $user_id ?>&puesto=supervisor" class="btn btn-sm btn-success">Aprobar</a>
																	<span class="bullet bullet-dot bg-success mx-2"></span></span>
